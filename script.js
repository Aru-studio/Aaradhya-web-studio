document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("hero-heading").innerHTML = data.settings.heroHeading; // may contain span
  document.getElementById("hero-sub").textContent = data.settings.heroSub;
  if(data.settings.logoUrl && data.settings.logoUrl.startsWith('data:image')) {
    document.getElementById("site-logo").src = data.settings.logoUrl;
    document.getElementById("site-logo-modal").src = data.settings.logoUrl;
  }
  
  var emailBtn = document.getElementById("contact-email-btn");
  if(emailBtn) emailBtn.href = "mailto:" + data.settings.contactEmail;
  var phoneBtn = document.getElementById("contact-phone-btn");
  if(phoneBtn) phoneBtn.href = "https://wa.me/" + data.settings.contactPhone.replace(/[\s+]/g, '');

  // Inject Stats
  var statsHtml = data.stats.map(s => `
    <div class="stat reveal show">
      <strong data-count="${s.count}">0</strong>
      <span>${s.label}</span>
    </div>
  `).join('');
  ...
  // Inject Projects
  var projectsHtml = data.projects.map(p => `
    <article class="card project reveal show" data-type="${p.type}">
  ...
  if(portfolioContainer) portfolioContainer.innerHTML = projectsHtml;
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      form.reset();
      alert("Message Sent Successfully! (This is a working demo without a real backend)");
    });
  }
